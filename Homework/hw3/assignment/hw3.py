...
def calc_binary_metrics(y_true_N, y_hat_N):
    ...
    n = len(y_true_N)
    for i in range(n):
        if y_true_N[i] and y_hat_N[i]:
            TP += 1 
        elif y_true_N[i] and not y_hat_N[i]:
            FN += 1
        elif not y_true_N[i] and not y_hat_N[i]:
            TN += 1
        elif not y_true_N[i] and y_hat_N[i]:
            FP += 1
    return TP, TN, FP, FN
...
def calc_percent_cancer(labels, cancer_label=1):
    ...
    """
    # DONE :: 
    count = np.count_nonzero(labels == cancer_label)
    return 100 * count / len(labels)
...
def calc_perf_metrics_for_threshold(y_true_N, y_proba1_N, thresh=0.5):
    ...
    decisionFromThreshold = [1 if x >= thresh else 0 for x in y_proba1_N]

    tp, tn, fp, fn = calc_binary_metrics(y_true_N, decisionFromThreshold)

    acc = accuracy_score(y_true_N, decisionFromThreshold)
...
def perceptron_classifier(x_train, y_train, x_test, y_test, penalty="l2",
    ...
    perceptron = Perceptron(penalty = penalty, alpha = alpha, shuffle = True, random_state = random_state)
    perceptron.fit(x_train, y_train)
    pred_train = perceptron.predict(x_train)
    pred_test  = perceptron.predict(x_test)

    '''
    trainMetrics = calc_binary_metrics(y_train, pred_train)
    testMetrics  = calc_binary_metrics(y_test, pred_test)

    # Accuracy of the data
    print("Train Accuracy: ", calc_accuracy(*trainMetrics))
    print("Test Accuracy: ", calc_accuracy(*testMetrics))

    print("Train Confusion Matrix:")
    print(calc_confusion_matrix_for_threshold(y_train, pred_train))
...
def series_of_preceptrons(x_train, y_train, x_test, y_test, alphas):
    ...
    for alpha in alphas:
        trainPrediction, testPrediction = perceptron_classifier(x_train, y_train, x_test, y_test,alpha = alpha)
        train_accuracy_list.append(accuracy_score(y_train, trainPrediction))
        test_accuracy_list.append(accuracy_score(y_test, testPrediction))
...
def calibrated_perceptron_classifier(x_train, y_train, x_test, y_test,
    ...
    perceptron = Perceptron(penalty = penalty, alpha = alpha, shuffle = True, random_state = random_state)
    perceptron.fit(x_train, y_train)
    pred_test  = perceptron.decision_function(x_test)

    calibrated = CalibratedClassifierCV(perceptron, method = "isotonic")
    calibrated.fit(x_train, y_train)
    calibratedTest = calibrated.predict_proba(x_test)

    return pred_test, calibratedTest[:, 1].flatten()
...
def find_best_thresholds(y_test, pred_prob_test):
    ...
    for threshold in np.linspace(0, 1.001, 51):
        _, tpr, _, ppv, _ = calc_perf_metrics_for_threshold(y_test, pred_prob_test, thresh = threshold)
